...
export default function TotalGreensCard() {
  ...
  const fetchGreens = async () => {
    ...
    const { count, error } = await supabase
      ...
      .eq('status', 'green') // filtra apenas green
  ...
  return (
    <div style={{
      ...
      <h2 style={{ fontSize: '1.1rem', marginBottom: '8px', color: '#374151' }}>Total de greens</h2>
      <p style={{ fontSize: '1.8rem', fontWeight: 'bold', color: total !== null ? (total >= 0 ? '#10b981' : '#ef4444') : '#111827' }}>
        {loading ? '...' : `${total}`}
    </p>
